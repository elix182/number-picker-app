...
const Conffeti = props => {
    //-----------Var Inits--------------
    const canvasRef = useRef();

    // Logic should be inside useEffect according to ESlint
    useEffect(() => {
        // Init constants
        const confetti = [];
        ...
        //---------Execution--------
        if(canvasRef.current && props.show){
            const canvas = canvasRef.current;
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            initConfetti(canvas);
    ...
    return props.show? <canvas className={className} ref={canvasRef}>{props.children}</canvas> : null;
}
